#include "Pch.h"
#include "EngineStatisticsWindow.h"
...
EngineStatisticsWindow::EngineStatisticsWindow() : SubWindow("Engine"), _fpsIt(0), _timeIt(0)
{
    _fpsValues.resize(maxFps);
    _timeValues.resize(maxTime);
}

EngineStatisticsWindow::~EngineStatisticsWindow()
{
}

void EngineStatisticsWindow::DrawWindowContent(const std::shared_ptr<CommandList>& commandList)
{
    ...
    ImGui::PlotHistogram("##fps", _fpsValues.data(), _fpsValues.size(), 0, currentFramerate, 0.f, 160.f, ImVec2(availableX, 160));
    ...
    ImGui::PlotLines("##latency", _timeValues.data(), _timeValues.size(), 0, currentTime, 0.f, 40.0f, ImVec2(availableX, 100));
